...
class NoRecursiveModel:
    ...
    def feature_engioneering(self,idx):
        ...
        # 比率の統計量
        rate_mbd_df = self.train.copy()
        rate_mbd_df.loc[rate_mbd_df["microbusiness_density"]==0, "microbusiness_density"] = 1
        rate_mbd_shift1 = rate_mbd_df.groupby("cfips")["microbusiness_density"].shift(1).fillna(method="bfill")
        rate_mbd_df["rate_mbd"] = rate_mbd_df["microbusiness_density"] / rate_mbd_shift1
        rate_mbd_groupby_mean = rate_mbd_df.groupby("cfips")["rate_mbd"].mean().reset_index()
        rate_mbd_groupby_mean = rate_mbd_groupby_mean.rename(columns={"rate_mbd":"rate_mbd_mean"})
        rate_mbd_groupby_std = rate_mbd_df.groupby("cfips")["rate_mbd"].std().reset_index()
        rate_mbd_groupby_std = rate_mbd_groupby_std.rename(columns={"rate_mbd":"rate_mbd_std"})
        
        
        #各種特徴量をマージ
        feature_df               = pd.merge(feature_df, rate_mbd_groupby_mean, on="cfips")
        feature_df               = pd.merge(feature_df, rate_mbd_groupby_std, on="cfips")
