class CV:
    ...
    def divide_data(self):
        ...
        if (self.cv_type == "None_ver2"):
            # Ref
            # https://www.kaggle.com/code/cdeotte/seasonal-model-with-validation-lb-1-091
            train_month = self.train["first_day_of_month"].values[:-5]
            val_month = self.train["first_day_of_month"].values[-5]
            submit_month = self.train["first_day_of_month"].values[-4:]
            ...
            # 返り値は、各データ（train, public, private）相当のインデックスを返す
        elif (self.cv_type == "from_202102_ver2"):
            """
            2021/02以降のデータで学習するようにデータを分割した分岐
            """
            train_month = self.train["first_day_of_month"].values[18:-5]  # 2021/2は、index=18なので、18以降を取得
            val_month = self.train["first_day_of_month"].values[-5]
            submit_month = self.train["first_day_of_month"].values[-4:]
            ...
            submit_month = self.train["first_day_of_month"].values[-5:-1] # 2023-02-01 - 2022-05-01 想定
