...
from catmodel_no_recursive import CATModel
...
pred_val = model.train_copy[( pd.to_datetime(model.train_copy['first_day_of_month']).isin(VAL_DATE))]
pred_sub = model.train_copy[( pd.to_datetime(model.train_copy['first_day_of_month']).isin(SUB_DATE[1:]))]
