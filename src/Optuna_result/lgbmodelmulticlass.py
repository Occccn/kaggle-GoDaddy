...
class LGBModel:
    def __init__(self,cfg):
        self.LGBCFG        = cfg
        self.LOG_EVAL      = 100
        self.NOT_FEATURES  = ['cfips','first_day_of_month','shift_target','microbusiness_density', 'row_id','label']
        self.TARGET        = 'label'
        self.DROP_COUNTY   = []
        self.PARAMS        = self.LGBCFG['params']
        self.CLASSIFIER    = pd.read_csv("../../data/raw/rate_shift1_add_label.csv")
        
        self.LABEL_MAP     = {0: 0.953,
                            1: 0.996,
                            2: 1,
                            3: 1.008,
                            4: 1.063,}
    ...
    def feature_engioneering(self):
        ...
        feature_df               = pd.merge(feature_df,self.CLASSIFIER[['row_id','label']] ,on = 'row_id',how = 'inner')
        feature_df               = pd.merge(feature_df,target[['row_id','shift_target','microbusiness_density']] ,on = 'row_id',how = 'inner')
    ...
    def train_model(self): 
        ...
        self.preds  = np.argmax(gbm.predict(self.mart_val.drop(columns = self.NOT_FEATURES)), axis=1)

    def update_train_data(self):
        self.mart_val['preds']     = self.preds
        self.mart_val['preds_rate'] = self.mart_val['preds'].map(self.LABEL_MAP)
        print(self.mart_val['preds'].value_counts())
        self.mart_val['preds_mbd'] = (self.mart_val['preds']) * self.mart_val['shift_target']
        self.train.loc[self.train['row_id'].isin(self.mart_val['row_id'].values),'microbusiness_density'] = ((self.mart_val['preds_rate']) * self.mart_val['shift_target']).values
