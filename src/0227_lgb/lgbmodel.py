...
class LGBModel:
    def __init__(self,cfg):
        self.LGBCFG        = cfg
        self.LOG_EVAL      = 100
        self.NOT_FEATURES  = ['cfips','first_day_of_month','target','shift_target','microbusiness_density', 'row_id','state']
        self.TARGET        = 'target'
        self.DROP_COUNTY   = []
    ...
    def feature_engioneering(self):
        feature_df = self.train[self.train['first_day_of_month'] >= pd.to_datetime('2021/2/1')][['cfips', 'first_day_of_month','row_id','state']].copy()
        ...
        for shift_month in range(1,6):
            ...
            lag_features[f'active_shift{shift_month}'] = lag_features.groupby(['cfips'])[['active']].shift(shift_month)
            lag_features_col.append(f'active_shift{shift_month}')
        ...
        for i in range(1,3):
            ...
            for size in [3, 6]:
                ...
                rolling_features[f"rolling_lag_min_t{size}_shift{DAYS_PRED}"] = rolling_features.groupby(['cfips'])['microbusiness_density'].transform(lambda x: x.shift(DAYS_PRED).rolling(size).min())
                rolling_features_col.append(f"rolling_lag_min_t{size}_shift{DAYS_PRED}")
                
        
        state_rolling_features = rolling_features.groupby('state')[rolling_features_col].mean().reset_index()
        col_list = ['state']
        for col in state_rolling_features.columns[1:]:
            col_list.append('state_' + col)
        state_rolling_features.columns = col_list
        #分散
        ...
        feature_df               = pd.merge(feature_df,state_rolling_features ,on = 'state',how = 'inner')
